import Key from "../i18nKey";
import type { Translation } from "../translation";

export const tr: Translation = {
	[Key.home]: "Anasayfa",
	[Key.about]: "Hakkında",
	[Key.archive]: "Arşiv",
	[Key.search]: "Ara",
	[Key.friends]: "Arkadaşlar",
	[Key.projects]: "Projeler",
	[Key.sponsor]: "Sponsor",
	[Key.sponsorButton]: "Sponsor Ol",
	[Key.sponsorButtonText]: "Bu yazı size yardımcı olduysa, destek olmayı düşünün!",
	[Key.copyLink]: "Bağlantıyı Kopyala",
	[Key.copied]: "Kopyalandı!",
	[Key.bangumi]: "Bangumi",
	[Key.bangumiLoading]: "Bangumi yükleniyor...",
	[Key.bangumiEmpty]: "Bangumi öğesi yok.",
	[Key.bangumiError]: "Bangumi yüklenemedi.",
	[Key.bangumiScore]: "Puan",
	[Key.bangumiUntitled]: "Başlıksız",
	[Key.devices]: "Cihazlar",
	[Key.devicesSubtitle]: "Günlük hayatta kullandığım cihazlar",
	[Key.devicesViewDetails]: "Detayları Gör",
	[Key.articles]: "Yazılar",

	[Key.toc]: "İçindekiler",
	[Key.tags]: "Taglar",
	[Key.categories]: "Katagoriler",

	[Key.comments]: "Yorumlar",

	[Key.uncategorized]: "Katagorisiz",
	[Key.noTags]: "Tag Bulunamadı",

	[Key.wordCount]: "kelime",
	[Key.wordsCount]: "kelime",
	[Key.minuteCount]: "dakika",
	[Key.minutesCount]: "dakika",
	[Key.postCount]: "gönderi",
	[Key.postsCount]: "gönderiler",

	[Key.themeColor]: "Tema Rengi",

	[Key.lightMode]: "Aydınlık",
	[Key.darkMode]: "Koyu",
	[Key.systemMode]: "Sistem",

	[Key.more]: "Daha Fazla",

	[Key.author]: "Yazar",
	[Key.publishedAt]: "Yayınlanma:",
	[Key.lastUpdatedAt]: "Son güncelleme:",
	[Key.license]: "Lisans",
};
