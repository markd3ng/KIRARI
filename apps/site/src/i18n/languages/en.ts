import Key from "../i18nKey";
import type { Translation } from "../translation";

export const en: Translation = {
	[Key.home]: "Home",
	[Key.about]: "About",
	[Key.archive]: "Archive",
	[Key.search]: "Search",
	[Key.friends]: "Friends",
	[Key.projects]: "Projects",
	[Key.sponsor]: "Sponsor",
	[Key.sponsorButton]: "Sponsor",
	[Key.sponsorButtonText]: "If this article helped you, please consider supporting me!",
	[Key.copyLink]: "Copy Link",
	[Key.copied]: "Copied!",
	[Key.bangumi]: "Bangumi",
	[Key.bangumiLoading]: "Loading Bangumi...",
	[Key.bangumiEmpty]: "No Bangumi items.",
	[Key.bangumiError]: "Failed to load Bangumi.",
	[Key.bangumiScore]: "Score",
	[Key.bangumiUntitled]: "Untitled",
	[Key.devices]: "Devices",
	[Key.devicesSubtitle]: "Here are the devices I use",
	[Key.devicesViewDetails]: "View Details",
	[Key.articles]: "Articles",

	[Key.toc]: "Table of Contents",
	[Key.tags]: "Tags",
	[Key.categories]: "Categories",

	[Key.comments]: "Comments",

	[Key.uncategorized]: "Uncategorized",
	[Key.noTags]: "No Tags",

	[Key.wordCount]: "word",
	[Key.wordsCount]: "words",
	[Key.minuteCount]: "minute",
	[Key.minutesCount]: "minutes",
	[Key.postCount]: "Post",
	[Key.postsCount]: "Posts",

	[Key.themeColor]: "Theme Color",

	[Key.lightMode]: "Light",
	[Key.darkMode]: "Dark",
	[Key.systemMode]: "System",

	[Key.more]: "More",

	[Key.author]: "Author",
	[Key.publishedAt]: "Published at",
	[Key.lastUpdatedAt]: "Last updated at",
	[Key.license]: "License",
};
