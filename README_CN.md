...
静态博客主题 — Astro 6.4.8 + Svelte 5 + Tailwind CSS v4。单一 TOML 配置驱动。支持 Cloudflare Pages、Vercel、Netlify、EdgeOne Pages 部署。
...
配置采用 **TOML-first**：普通站点配置写在
`packages/site-profile/kirari.config.toml`，构建时会 materialize 到
`apps/site/kirari.config.toml`。环境变量只用于密钥、部署差异，以及不适合提交到仓库的服务凭据。
...
| 代码高亮 | astro-expressive-code + 4 插件 | ^0.43.1 |
...
`apps/site/src/styles/` 或 `apps/site/src/utils/` 下的程序文件。
...
`packages/site-profile/kirari.config.toml` 中每个键都带有中英双语注释。文件本身即为 profile 的完整参考。以下为结构概览。
...
`apps/site/dist/` 即可。部署到 `https://user.github.io/repo/` 时，在
`packages/site-profile/kirari.config.toml` 中设置 `site.base = "/repo/"`；使用自定义域名时保持
`site.base = "/"`，并在 Pages 设置中绑定域名。Pagefind、RSS、sitemap 与 LLMs
文件都会在正常构建中生成，因此应部署最终 `apps/site/dist/` 产物，而不是源码目录。

## 脚本命令

| 命令 | 执行 |
|---|---|
| `pnpm site:dev` | `@kirari/site` 的 `astro dev` |
| `pnpm build` | materialize → astro build → postbuild |
| `pnpm check` | site type-check → Astro check → edge type-check → edge test |
| `pnpm site:type-check` | `@kirari/site` 的 `tsc --noEmit` |
| `pnpm site:astro-check` | `@kirari/site` 的 `astro check` |
| `pnpm edge:type-check` | `@kirari/edge` 的 `tsc --noEmit` |
| `pnpm edge:test` | edge runtime 测试 |
| `pnpm --filter @kirari/site preview` | `astro preview` |
| `pnpm --filter @kirari/site new-post` | 交互式创建文章 |
...
├── apps/
│   └── site/                   # Astro 站点程序
│       ├── kirari.config.toml  # materialize 后的 profile 配置
...
├── packages/
│   └── site-profile/           # 默认内容、配置、品牌资产、snippets
├── workers/
│   └── kirari-edge/            # 可选 edge runtime
└── apps/site/dist/             # 构建输出（gitignored）
```
